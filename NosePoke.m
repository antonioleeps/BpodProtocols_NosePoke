function NosePoke()
% Learning to Nose Poke side ports - Switch, Serial Action learning
...
if isempty(fieldnames(TaskParameters))
    ...
    TaskParameters.GUI.CenterPortRewAmount = 8;
    TaskParameters.GUI.CenterPortProb = 1;
    TaskParameters.GUI.FirstActionReward =10;
...
InitValveTime_Left = GetValveTimes(TaskParameters.GUI.FirstActionReward, LeftPort);
InitValveTime_Right = GetValveTimes(TaskParameters.GUI.FirstActionReward, RightPort);
...
sma = SetGlobalTimer(sma,3,TaskParameters.GUI.ChoiceDeadline);
...
sma = AddState(sma, 'Name', 'wait_Sin',...
    ...
    'OutputActions',{strcat('PWM',num2str(LeftPort)),LeftLight,strcat('PWM',num2str(RightPort)),RightLight, 'GlobalTimerTrig',3});


sma = AddState(sma, 'Name', 'wait_L_start',...
    'Timer',InitValveTime_Left,...
    'StateChangeConditions', {'Tup','wait_L','GlobalTimer3_End', 'ITI', LeftPortOut,'wait_L_grace'},...
    'OutputActions',{'GlobalTimerTrig',2, 'ValveState', LeftValve, strcat('PWM',num2str(LeftPort)),LeftLight,strcat('PWM',num2str(RightPort)),RightLight});

sma = AddState(sma, 'Name', 'wait_L',...
    'Timer',DelayTime,...
    'StateChangeConditions', {LeftPortOut,'wait_L_grace','GlobalTimer3_End', 'ITI'},...
...
sma = AddState(sma, 'Name', 'wait_R',...
    'Timer',DelayTime,...
    'StateChangeConditions', {RightPortOut,'wait_R_grace', 'GlobalTimer3_End', 'ITI'},...
    'OutputActions',{strcat('PWM',num2str(LeftPort)),LeftLight,strcat('PWM',num2str(RightPort)),RightLight});

sma = AddState(sma, 'Name', 'wait_R_grace',...
    'Timer',DelayTime,...
    'StateChangeConditions', {LeftPortIn, LeftWaitAction, 'GlobalTimer3_End', 'ITI'},...
    'OutputActions',{strcat('PWM',num2str(LeftPort)),LeftLight,strcat('PWM',num2str(RightPort)),RightLight});
